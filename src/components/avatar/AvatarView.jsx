...
// Décors Mario-style lumineux par niveau
function Environment({ level }) {
  const W = 220, H = 110;
  if (level === 'lionceau') return (
    // Prairie Mario — ciel bleu, herbe verte
    <svg width={W} height={H} style={{ imageRendering: 'pixelated', display: 'block', width: '100%' }}>
      ...
      {[[20,15],[140,20],[70,8]].map(([x,y],i) => (
        <rect key={i} x={x} y={y} width={30} height={12} rx={0} fill="rgba(255,255,255,0.9)" />
      ))}
      {/* Sol */}
    ...
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '0 16px 8px' }}>
        ...
        <div style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          {level === 'legendaire' && (
            <div style={{
              ...
              animation: 'pxBlink 1s infinite', color: '#F8D000',
            }}>★</div>
          )}
        </div>
      ...
      <div className="card animate-fade-up delay-4" style={{ padding: '18px 20px', marginBottom: 20, borderColor: `${SEC}88` }}>
        ...
        {[
          ...
          { lvl: 'LVL 02', name: 'ADULTE',    range: '50-199 pts', color: '#FC5252', reached: score >= 50 },
          { lvl: 'LVL 03', name: 'LEGENDAIRE',range: '200+ pts',   color: '#F8D000', reached: score >= 200 },
